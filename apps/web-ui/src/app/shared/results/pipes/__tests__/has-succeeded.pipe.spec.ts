import { Result, toFailure, toSuccess } from '../../results';
import { HasSucceededPipe } from '../has-succeeded.pipe';

describe('HasSucceededPipe', () => {
  let pipe: HasSucceededPipe;

  beforeEach(() => {
    pipe = new HasSucceededPipe();
  });

  it('should return true for a successful result', () => {
    const successResult: Result<number> = toSuccess(42);
    expect(pipe.transform(successResult)).toBe(true);
  });

  it('should return false for a failed result', () => {
    const failedResult: Result<number> = toFailure(new Error('Test error'));
    expect(pipe.transform(failedResult)).toBe(false);
  });

  it('should return false for a pending result', () => {
    const pendingResult: Result<number> = {
      data: undefined,
      isLoading: true,
      error: undefined,
    };
    expect(pipe.transform(pendingResult)).toBe(false);
  });
});
